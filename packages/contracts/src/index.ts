...
export const classificationSchema = z.enum(['PUBLIC', 'INTERNAL', 'PERSONAL', 'SENSITIVE', 'HIGHLY_SENSITIVE']);
...
export const eventEnvelopeSchema = z.strictObject({
  ...
  event_type: z.string().regex(/^[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*\.v[1-9][0-9]*$/),
  ...
  trace_id: z.string().regex(/^[a-f0-9]{32}$/).refine((v) => v !== '0'.repeat(32)),
  ...
  if (!event.event_type.endsWith(`.v${event.event_version}`)) {
    ctx.addIssue({ code: 'custom', message: 'Event name/version mismatch', path: ['event_version'] });
  }
});
...
export const apiErrorSchema = z.strictObject({
  error: z.strictObject({ code: z.string().min(1), message: z.string().min(1), details: z.record(z.string(), z.unknown()) }),
...
export const aiCapabilitySchema = z.enum(['TRANSLATION', 'CLASSIFICATION', 'SUMMARIZATION', 'EMBEDDING', 'SPEECH_TO_TEXT', 'ANSWER_ASSISTANCE']);
...
export const aiProviderConfigSchema = z.strictObject({
  ...
  base_url: z.string().url().refine((value) => { const u = new URL(value); return u.protocol === 'https:' && !u.username && !u.password && !u.search && !u.hash; }),
