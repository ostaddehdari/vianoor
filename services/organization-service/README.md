# سازمان | organization-service

وضعیت: **اسکلت مرحلهٔ ۱**؛ منطق دامنه در مرحلهٔ 6 اضافه می‌شود.

مالکیت: سازمان، شعبه، عضویت و مجوزهای محدوده‌ای

Database: `organization_db`؛ role: `organization_app`. هیچ migration نمایشی اجرا نمی‌شود.

از ریشه: `npm run build:packages` سپس `npm run dev --workspace @vianoor/organization-service`. پورت پیش‌فرض 4103.
