# پشتیبانی | support-service

وضعیت: **اسکلت مرحلهٔ ۱**؛ منطق دامنه در مرحلهٔ 14 اضافه می‌شود.

مالکیت: تیکت، مکاتبه پشتیبانی، ارجاع و پیگیری شکایت

Database: `support_db`؛ role: `support_app`. هیچ migration نمایشی اجرا نمی‌شود.

از ریشه: `npm run build:packages` سپس `npm run dev --workspace @vianoor/support-service`. پورت پیش‌فرض 4129.
