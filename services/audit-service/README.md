# حسابرسی | audit-service

وضعیت: **اسکلت مرحلهٔ ۱**؛ منطق دامنه در مرحلهٔ 6 اضافه می‌شود.

مالکیت: سوابق append-only عملیات حساس و دسترسی

Database: `audit_db`؛ role: `audit_app`. هیچ migration نمایشی اجرا نمی‌شود.

از ریشه: `npm run build:packages` سپس `npm run dev --workspace @vianoor/audit-service`. پورت پیش‌فرض 4128.
