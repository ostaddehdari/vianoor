# محتوا و کانال | content-service

وضعیت: **اسکلت مرحلهٔ ۱**؛ منطق دامنه در مرحلهٔ 16 اضافه می‌شود.

مالکیت: کانال، دنبال‌کردن، پست، مجموعه و انتشار

Database: `content_db`؛ role: `content_app`. هیچ migration نمایشی اجرا نمی‌شود.

از ریشه: `npm run build:packages` سپس `npm run dev --workspace @vianoor/content-service`. پورت پیش‌فرض 4119.
