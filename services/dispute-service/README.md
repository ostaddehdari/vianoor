# اختلافات و بازپرداخت | dispute-service

وضعیت: **اسکلت مرحلهٔ ۱**؛ منطق دامنه در مرحلهٔ 11 اضافه می‌شود.

مالکیت: پرونده اختلاف، تصمیم و orchestration بازپرداخت

Database: `dispute_db`؛ role: `dispute_app`. هیچ migration نمایشی اجرا نمی‌شود.

از ریشه: `npm run build:packages` سپس `npm run dev --workspace @vianoor/dispute-service`. پورت پیش‌فرض 4118.
