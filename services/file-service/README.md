...
مالکیت: metadata، مالکیت، دسترسی و وضعیت اسکن؛ bytes در object storage

Database: `file_db`؛ role: `file_app`. هیچ migration نمایشی اجرا نمی‌شود.

از ریشه: `npm run build:packages` سپس `npm run dev --workspace @vianoor/file-service`. پورت پیش‌فرض 4113.
