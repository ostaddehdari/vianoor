# دسترس‌پذیری | availability-service

وضعیت: **اسکلت مرحلهٔ ۱**؛ منطق دامنه در مرحلهٔ 9 اضافه می‌شود.

مالکیت: برنامه کاری، استثنا، slot و نگهداری اتمیک ظرفیت

Database: `availability_db`؛ role: `availability_app`. هیچ migration نمایشی اجرا نمی‌شود.

از ریشه: `npm run build:packages` سپس `npm run dev --workspace @vianoor/availability-service`. پورت پیش‌فرض 4106.
