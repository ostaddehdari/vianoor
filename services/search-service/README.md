# جست‌وجو | search-service

وضعیت: **اسکلت مرحلهٔ ۱**؛ منطق دامنه در مرحلهٔ 10 اضافه می‌شود.

مالکیت: نمایه قابل بازسازی و checkpoint؛ نه مالک محتوای اصلی

Database: `search_db`؛ role: `search_app`. هیچ migration نمایشی اجرا نمی‌شود.

از ریشه: `npm run build:packages` سپس `npm run dev --workspace @vianoor/search-service`. پورت پیش‌فرض 4126.
