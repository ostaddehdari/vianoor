...
مالکیت: intent، تراکنش درگاه، verification و idempotency

Database: `payment_db`؛ role: `payment_app`. هیچ migration نمایشی اجرا نمی‌شود.

از ریشه: `npm run build:packages` سپس `npm run dev --workspace @vianoor/payment-service`. پورت پیش‌فرض 4114.
