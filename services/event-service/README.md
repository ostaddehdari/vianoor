# رویداد و منبر | event-service

وضعیت: **اسکلت مرحلهٔ ۱**؛ منطق دامنه در مرحلهٔ 17 اضافه می‌شود.

مالکیت: رویداد، ظرفیت، ثبت‌نام، نقش‌ها، نظرسنجی و بازپخش

Database: `event_db`؛ role: `event_app`. هیچ migration نمایشی اجرا نمی‌شود.

از ریشه: `npm run build:packages` سپس `npm run dev --workspace @vianoor/event-service`. پورت پیش‌فرض 4120.
