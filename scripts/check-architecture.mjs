...
assert(new Set(requirements.map((r) => r.id)).size === requirements.length, 'Duplicate requirement IDs');
assert(new Set(pages.map((p) => `${p.app}:${p.path}`)).size === pages.length, 'Duplicate routes');
for (const page of pages) {
  assert(catalog.some((s) => s.id === page.dataOwner), `${page.id}: unknown owner`);
  assert(page.path.startsWith('/[locale]/'), `${page.id}: missing locale`);
}
const ids = new Set(catalog.map((s) => s.id));
assert(readdirSync('services').sort().join() === [...ids].sort().join(), 'Service directory/catalog mismatch');
for (const service of catalog) {
  ...
  assert(main.includes(`'${service.id}'`) && main.includes(String(service.port)), `Entry/catalog drift: ${service.id}`);
}
  ...
  for (const dir of readdirSync(group)) {
    ...
    for (const key of ['dependencies','devDependencies','peerDependencies']) {
      for (const [dep,version] of Object.entries(pkg[key] ?? {})) {
        ...
        assert(!dep.startsWith('@vianoor/') || ['@vianoor/contracts','@vianoor/service-runtime','@vianoor/ui'].includes(dep), `Domain package dependency forbidden: ${dep}`);
      }
    }
  }
}
if (errors.length) { console.error(errors.join('\n')); process.exit(1); }
console.log(`Architecture OK: ${catalog.length} services, ${requirements.length} requirements, ${pages.length} planned pages.`);
