...
if (!['build', 'typecheck'].includes(task) || (selected && !['packages','services','apps'].includes(selected))) throw new Error('Invalid workspace task');
for (const group of selected ? [selected] : ['packages', 'services', 'apps']) {
  for (const directory of readdirSync(group).sort()) {
    ...
    const result = spawnSync('npm', ['run', task, '--workspace', pkg.name], { stdio: 'inherit', shell: process.platform === 'win32' });
